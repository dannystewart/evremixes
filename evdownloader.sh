...
if [ -n "$URL" ]; then
    ...
    curl -o "${TEMP_DIR}/evremixes" -L "$URL" --progress-bar
...
# Fall back to the less cool Bash version if the Python version fails
OUTPUT_FOLDER="$HOME/Downloads/Evanescence Remixes"
KILL_SWITCH=0

# Welcome message
echo -e "${YELLOW}Couldn't run Python version. Falling back to Bash version.${NC}"
echo ""
